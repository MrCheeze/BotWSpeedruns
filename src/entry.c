...
int __entry_menu(int argc, char **argv)
{
		...
		|| OSGetTitleID() == 0x0005001010040000)) //Wii U Menu JPN
	{
		...
		InitPadScoreFunctionPointers(); //for Pro Controller
		...
		OSGetTitleID() != 0x000500101004A000)   // mii maker jpn)
	{
		...
		if (OSGetTitleID() != 0 && (OSGetTitleID() == 0x0005000010143500 || OSGetTitleID() == 0x0005000010143600 || OSGetTitleID() == 0x0005000010143400)) //TWW HD is running
			...
			g_gameID = 1;
		else if (OSGetTitleID() != 0 && (OSGetTitleID() == 0x00050000101C9300 || OSGetTitleID() == 0x00050000101C9400 || OSGetTitleID() == 0x00050000101C9500)) //BotW is running
			g_gameID = 2;
